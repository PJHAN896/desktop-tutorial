...
- "📹 카메라로 연습하기" 버튼을 누르면 답변하는 동안 얼굴과 목소리가 함께 녹화되고, 답변을 제출하면 방금 녹화한 영상(소리 포함)을 바로 다시 볼 수 있습니다. 동시에 목소리 볼륨을 분석해 침묵 비율, 목소리 크기, 톤의 강약에 대한 피드백도 함께 표시됩니다(말의 내용이 아니라 소리 특성을 분석하는 방식입니다). 카메라는 완전히 선택 사항이라 버튼을 안 누르거나 권한을 거부해도 나머지 기능은 그대로 동작하며, 영상은 저장되지 않고 그 문항을 벗어나면 사라집니다.
